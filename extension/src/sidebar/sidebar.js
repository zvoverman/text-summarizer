...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Send summarization message when button is clicked
    summarizeBtn.addEventListener('click', function () {
        ...
        browser.tabs.query({ active: true, currentWindow: true })
            .then(tabs => {
                ...
                browser.tabs.sendMessage(tabId, { action: 'summarizeBtnClicked', timeStamp: Date.now() })
                    .then((response) => { 
    ...
    browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
        if (request.action === 'summarizeResponse') {
            ...
            document.getElementById('timeElapsedValue').textContent = (request.timeElapsed / 1000) + " sec";
            summarizeBtn.innerHTML = "Summarize";
            summarizeBtn.disabled = false;
